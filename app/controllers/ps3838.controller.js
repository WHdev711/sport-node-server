...
exports.GetOdds = async (req, res) => {
    ...
    var eventlist = [];
    allodds = await axios
        .get(`${PS3838Config.ODDS}?${PS3838Config.Soccer}`, { headers: { 'Authorization': `Basic ${PS3838Config.HeaderBasic}` } })
        .then(response => {
    ...
    for (var h = 0; h <= div_count; h++) {
        var _tmpfixture = await axios
            .get(`${PS3838Config.FIXTURES}?${PS3838Config.Soccer}&eventIds=${eventlist.slice(h * 500, (h + 1) * 500).toString()}`, { headers: { 'Authorization': `Basic ${PS3838Config.HeaderBasic}` } })
            .then(response => {
